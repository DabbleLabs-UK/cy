...
import { tempoIdleMs, clampSpeed } from './tempo.js';
...
// ---- 17. tempo as a literal generation duty cycle --------------------------
hr('17. TEMPO DUTY CYCLE (exact idle between bursts)');
line('speed=100 -> continuous (no idle): ' + tempoIdleMs(1000, 100) + ' ms (expect 0)');
line('duty maths: 50->burst, 25->3x burst: ' +
  (tempoIdleMs(1000, 50) === 1000 && tempoIdleMs(1000, 25) === 3000));
line('speed=30, burst 2000ms -> ' + tempoIdleMs(2000, 30) + ' ms idle');
line('someone watching (30%), burst 75s -> idle ' + tempoIdleMs(75000, 30) + ' ms, cadence ~' +
  Math.round((75000 + tempoIdleMs(75000, 30)) / 1000) + 's');
line('30% remains 30% for a long burst: ' +
  (Math.round(100 * 75000 / (75000 + tempoIdleMs(75000, 30))) === 30));
line('nobody watching (5%), burst 75s -> idle ' + tempoIdleMs(75000, 5) + ' ms, cadence ~' +
  Math.round((75000 + tempoIdleMs(75000, 5)) / 60000) + ' min');
line('5% remains 5% for a long burst: ' +
  (Math.round(100 * 75000 / (75000 + tempoIdleMs(75000, 5))) === 5));
