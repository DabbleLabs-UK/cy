...
import { reconcileSoma, groundedSomaDirective, somaSnapshot } from './soma.js';
import { buildDirectives, options } from './prompt.js';
import { brainRegions, heartRate, initialVitals } from './vitals.js';
...
const somaLow = reconcileSoma(null, { now: 1 });
const somaHigh = reconcileSoma(JSON.parse(JSON.stringify(somaLow)), { now: 1 });
somaLow.experienced.metrics.anxiety.value = 0;
somaHigh.experienced.metrics.anxiety.value = 100;
assert.equal(somaSnapshot(somaLow).operationalAnxiety.status, somaSnapshot(somaHigh).operationalAnxiety.status,
  'L: legacy scalar cannot change the operational headline');
assert.equal(groundedSomaDirective(somaLow).directive, groundedSomaDirective(somaHigh).directive,
  'L/P: legacy scalar cannot change grounded prose context');

const lowVitals = initialVitals();
const highVitals = JSON.parse(JSON.stringify(lowVitals));
lowVitals.mental.anxiety = 0;
highVitals.mental.anxiety = 1;
const contextDirectives = { incidents: 'same incident', groundedSoma: 'same grounded facts' };
assert.equal(buildDirectives(lowVitals, 'journal', contextDirectives),
  buildDirectives(highVitals, 'journal', contextDirectives), 'L/P: legacy scalar cannot change prose');
assert.deepEqual(options(lowVitals, 2, 'journal'), options(highVitals, 2, 'journal'),
  'L/P: legacy scalar cannot change sampling or output timing');
assert.deepEqual(brainRegions(lowVitals), brainRegions(highVitals),
  'L/N: legacy scalar cannot change brain-region activation');
assert.equal(heartRate(lowVitals), heartRate(highVitals),
  'L: legacy scalar is isolated from the compatibility body display');

assert.equal(implementationEntry('soma_variables', 'anxiety').implementation_status, 'IMPLEMENTED');
assert.equal(implementationEntry('soma_variables', 'legacy_anxiety_scalar').diagnostics_only, true);
assert.equal(implementationEntry('soma_variables', 'subjective_anxiety_magnitude').implementation_status,
  'NOT_IMPLEMENTED');
assert.equal(implementationEntry('soma_subsystems', 'temporal_threat_hazard').implementation_status,
  'NOT_IMPLEMENTED');
assert.equal(implementationEntry('soma_subsystems', 'amygdala_neural_activation').implementation_status,
  'NOT_IMPLEMENTED');
assert.equal(implementationEntry('soma_subsystems', 'bnst_neural_activation').implementation_status,
  'NOT_IMPLEMENTED');
assert.equal(implementationEntry('brain_regions', 'amygdala').implementation_status, 'PROVISIONAL');
assert.equal(implementationEntry('brain_regions', 'bnstUncertainThreat').implementation_status,
  'NOT_IMPLEMENTED', 'N: neural activation remains unavailable');

const source = await readFile(new URL('../public/assets/brain.js', import.meta.url), 'utf8');
assert.match(source, /operationalAnxietyHistoryMarkup/);
assert.match(source, /THREAT_ONGOING/);
